...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\JLU6\\Desktop\\Tobii testing\\gaze_contingent\\FANCY_CARDS.py',
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if t >= 0.0 and CARD1.status == NOT_STARTED:
            ...
            CARD1.tStart = t  # not accounting for scr refresh
            CARD1.frameNStart = frameN  # exact frame index
            win.timeOnFlip(CARD1, 'tStartRefresh')  # time at next scr refresh
            CARD1.setAutoDraw(True)
        
        # *CARD2* updates
        if t >= 0.0 and CARD2.status == NOT_STARTED:
            # keep track of start time/frame for later
            CARD2.tStart = t  # not accounting for scr refresh
            CARD2.frameNStart = frameN  # exact frame index
            win.timeOnFlip(CARD2, 'tStartRefresh')  # time at next scr refresh
            CARD2.setAutoDraw(True)
        
        # *CARD3* updates
        if t >= 0.0 and CARD3.status == NOT_STARTED:
            # keep track of start time/frame for later
            CARD3.tStart = t  # not accounting for scr refresh
            CARD3.frameNStart = frameN  # exact frame index
            win.timeOnFlip(CARD3, 'tStartRefresh')  # time at next scr refresh
    ...
    if expInfo['Eye Tracker']:
        my_eyetracker.unsubscribe_from(tr.EYETRACKER_GAZE_DATA, gaze_data_callback)
    trials.addData('CARD1.started', CARD1.tStartRefresh)
    trials.addData('CARD1.stopped', CARD1.tStopRefresh)
    trials.addData('CARD2.started', CARD2.tStartRefresh)
    trials.addData('CARD2.stopped', CARD2.tStopRefresh)
    trials.addData('CARD3.started', CARD3.tStartRefresh)
    trials.addData('CARD3.stopped', CARD3.tStopRefresh)
